...
const Footer = () => {
    ...
    <div className={`flex flex-col w-full`}>
        <div className='flex lg:grid flex-col lg:grid-cols-[1fr,1fr,1fr,1fr] lg:gap-3 w-full px-X-footer py-Y-footer bg-[#1a1a1a]'>
            ...
            <Link className='text-[1.2rem] 2xl:text-[1.3rem] text-w-1 font-semibold capitalize tracking-wide hover:text-g-1 border-2 px-2 py-1 rounded-md border-transparent hover:border-g-1 w-fit' >Contractual Placements</Link>
            <Link className='text-[1.2rem] 2xl:text-[1.3rem] text-w-1 font-semibold capitalize tracking-wide hover:text-g-1 border-2 px-2 py-1 rounded-md border-transparent hover:border-g-1 w-fit' >Permanents Placements</Link>
            ...
            <div className='flex flex-col py-2 px-X-footer lg:py-0 lg:flex-row justify-center'>
                {
                    FooterPages.map((Child, index) => (
                        <Link className='text-[1.1rem] lg:text-[1.2rem] text-w-1 mx-4 my-2 lg:my-0 lg:mx-10 hover:text-g-1 hover:underline underline-offset-4 lg:leading-tight'  key={index} path={Child.Link}>{Child.Name}</Link>
                    ))
                }
            </div>
